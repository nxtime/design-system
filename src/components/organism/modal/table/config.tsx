...
const TableConfigModal = <T extends TTableConfig<T>>({
  ...
  updateHiddenColumns: Dispatch<React.SetStateAction<(keyof T)[]>>;
  translation?: string;
  translationPrefix?: string;
}) => {
  const [internalConfig, updateInternalConfig] = useState(config);
  const [currentActiveTab, updateCurrentActiveTab] = useState("hide");
  ...
  return (
    <Modal currentName="table-config-modal" size="fill">
      <header className="group rounded-box">
        {tabs.map((tab) => {
          return (
            <button
              key={tab}
              className={`btn no-bounce ring-secondary btn-${
                currentActiveTab === tab ? "primary" : "neutral"
              }`}
              onClick={() => {
                if (currentActiveTab !== tab) updateCurrentActiveTab(tab);
              }}
              style={{ textTransform: "capitalize", width: "auto", flex: 1 }}
            >
              {translate(`settings.tabs.${tab as "hide"}`)}
            </button>
          );
        })}
      </header>
      {currentActiveTab === "hide" && (
        <div className="container">
          {(Object.keys(internalConfig) as (keyof T)[]).map((item) => {
            ...
            return (
              <div className="container row" style={{ alignItems: "center" }}>
                <input
                  ...
                  name={item as string}
                  id={item as string}
                  checked={!hiddenColumns.includes(item)}
                  className="checkbox checkbox-base-200"
                  onChange={() => {
                    console.log(hiddenColumns, item);
                    updateHiddenColumns((prev) => {
                      if (prev.includes(item)) {
                        return prev.filter((column) => column !== item);
                      }
                      return [...prev, item];
                    });
                  }}
